...
class LinkedBlockingDeque<E> extends AbstractQueue<E> implements Deque<E>, Serializable {
  ...
  /**
   * Links the provided element as the first in the queue, waiting until there is space to do so if
   * the queue is full.
   *
  ...
  /**
   * Links the provided element as the first in the queue, waiting up to the specified time to do so
  ...
  /**
   * Links the provided element as the last in the queue, waiting up to the specified time to do so
   * if the queue is full.
   *
   * @param e element to link
   * @param timeout length of time to wait
  ...
  /**
   * Unlinks the last element in the queue, waiting until there is an element to unlink if the queue
   * is empty.
   *
   * @return the unlinked element
   * @throws InterruptedException if the current thread is interrupted
  ...
  /**
   * Unlinks the first element in the queue, waiting up to the specified time to do so if the queue
   * is empty.
   *
   * @param timeout length of time to wait
   * @param unit units that timeout is expressed in
   ...
   * returned in order from first (head) to last (tail). The returned {@code Iterator} is a "weakly
   * consistent" iterator that will never throw {@link java.util.ConcurrentModificationException
  ...
  /**
   * Returns true if there are threads waiting to take instances from this deque. See disclaimer on
   * accuracy in {@link java.util.concurrent.locks.ReentrantLock#hasWaiters(Condition)}.
   *
  ...
  /**
   * Returns the length of the queue of threads waiting to take instances from this deque. See
  ...
  /**
   * This sub-class was created to expose the waiting threads so that they can be interrupted when
   * the pool using the queue that uses this lock is closed. The class is intended for internal use
   * only.
   *
